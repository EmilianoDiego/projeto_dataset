...
class Ui_MainWindow(object):
    ...
    def opcao_selecionada(self):
        ...
        regioes = self.comboBox_regioes.currentText()

        # verifica se os itens no combobox foram selecionados
        if (tabela == "selecione") or (regioes == "selecione"):
            ...
            dataset = pd.read_csv("projeto_dataset/csv/" + tabela, encoding="utf-8")
        ...
        sudeste = ["Diadema", "Mauá", "Ribeirão Pires", "Rio Grande da Serra", "Santo André", "São Bernardo do Campo",
                   "São Caetano do Sul"]
        ...
        dados = dataset[(dataset.Natureza == regioes)].sort_values("Ano")
        ...
        plt.title(regioes, fontsize=14)
        plt.xlabel("Ano", fontsize=10)
        plt.ylabel("numeros de:" + regioes, fontsize=10);
        # apresenta o grafico
        plt.show()
...
def setupUi(self, MainWindow):
    ...
    self.comboBox_regioes = QtWidgets.QComboBox(self.frame_inferior2)
    self.comboBox_regioes.setGeometry(QtCore.QRect(290, 40, 230, 20))
    self.comboBox_regioes.setStyleSheet("QComboBox{\n"
                                        "background-color: rgb(255, 255, 255);\n"
                                        "    background-color: rgb(232, 232, 232);\n"
                                        "}")
    self.comboBox_regioes.setObjectName("comboBox_regioes")
    self.comboBox_regioes.addItems(["selecione",
                                    "Região Leste",
                                    "Região Norte",
                                    "Região Sudeste",
                                    "Região Sudoeste",
...
def retranslateUi(self, MainWindow):
    ...
    self.label_natureza.setText(_translate("MainWindow", "Selecionar Região"))
    self.label_titulo.setText(_translate("MainWindow", "Consulta de criminalidade por região"))
...
dataset[(dataset.Cidade == "S�o Paulo")].sort_values("Ano")
